...
model = torch.load("../output/quant_full/quant_8b_full.pt")
model.cuda("cuda:2")
...
outputs = []
os.makedirs('../output/pred_quant_combine_pdb_v1/', exist_ok=True)
with torch.no_grad():
...
for identifier, pdb in zip(key_list, pdb_list):
    with open(f"../output/pred_quant_combine_pdb_v1/{identifier}.pdb", "w") as f:
        f.write("".join(pdb))
...
pred_quant_pdbs = os.listdir("../output/pred_quant_combine_pdb_v1")
...
for a, b in tqdm(zip(real_pdbs, pred_quant_pdbs)):
    tmscore(os.path.join("../data/cameo_real_pdb", a), os.path.join("../output/pred_quant_combine_pdb_v1", b))
